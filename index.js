...
const scheduleJob = async () => {
    try {
        const fortyFiveMinutesAgo = new Date(Date.now() - 45 * 60 * 1000); // 45 minutes ago

        // Find and delete documents with empty arrays and older than 45 minutes
        const result = await model.deleteMany({
            arr: { $size: 0 },
